...
newtype Select a = Select (State SelectState a)
...
run :: HashMap ByteString Int -> Select a -> (a, SelectState)
run used (Select s) =
...
subquery :: MonadQuery query => State SelectState a -> query (a, Raw.Select)
subquery q = do
  used <- getUsedNames
